...
/*
	...
	The rendering materials are slightly inspired by the Steampunk genre. Timber, brass, etc. It needs 
	spinning turbines, gears, rivots, and so forth, but that stuff's expensive. Maybe later. Tambako
	Jaguar did a really cool shader in the Steampunk aesthetic. The link is below.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    if(t<FAR){
		...
		col = vec3(0.5,0.5,0.5); // tex3D(iChannel0, pos*ts, nor); // Texture pixel at the scene postion.
        
        
        vec3  li = lp - pos; // Point light.
        float lDist = max(length(li), .001); // Surface to light distance.
        float atten = 1./(1.0 + lDist*0.125 + lDist*lDist*.025); // Light attenuation.
        ...
        float rAtten = 1./(1. + rlDist*0.125 + rlDist*rlDist*.025);
        
        if(rSaveObjID>1.5 && rSaveObjID<2.5){
            rCol = vec3(1)*dot(rCol, vec3(.299, .587, .114))*.8+.2;//*.7+.2
            //rDiff *= 1.35;
        }
        if(rSaveObjID>2.5){
             float rc = dot(rCol, vec3(.299, .587, .114));
             vec3 rFire = vec3(pow(rc, 8.), rc*rc, min(rc*1.5, 1.));//*.5+rc*.5;
             rCol = min(mix(vec3(1, .9, .375), vec3(.75, .375, .3), rFire), 1.);         
        }
        ...
        if(saveObjID>1.5 && saveObjID<2.5){ 
            ...
            col = vec3(1)*dot(col, vec3(.299, .587, .114))*.8+.2;
        ...
        if(saveObjID>2.5){
            ...
            vec3 fire = vec3(pow(c, 8.), c*c, min(c*1.5, 1.));//*.5+c*.5;
            col = min(mix(vec3(1, .9, .375), vec3(.75, .375, .3), fire), 1.);//col*.66;//
