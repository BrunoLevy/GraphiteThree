...
#ifndef H_OGF_SCENE_GRAPH_NL_BLAS_H
#define H_OGF_SCENE_GRAPH_NL_BLAS_H

#include <OGF/scene_graph/common/common.h>
#include <OGF/gom/types/object.h>

struct NLBlas;
typedef NLBlas* NLBlas_t;
...
namespace OGF {
	...
	/**
	 * \brief Wrapper class for BLAS operations on vectors (Basic Linear
	 *  Algebra).
	 */
	gom_class SCENE_GRAPH_API Blas : public Object {
	  public:
	    /**
	     * \brief Blas constructor.
	     */
	    Blas();
	    
	    /**
	     * \brief Blas destructor.
	     */
	     ~Blas() override;
	  ...
	  gom_slots:
	     ...
	     * \details Equivalent to sqrt(dot(x,x))
	     ...
	     * \param[in,out] y the vector that a x should be added to
	     */
	...
	/**
	 * \brief A reference-counted pointer to a Blas.
	 */
	typedef SmartPointer<Blas> Blas_var;
    }
}
