...
int Language::functionHandler(Node *n) {
  ...
  int isstatic = CurrentClass && Swig_storage_isstatic(n) && !(SmartPointerEmit && Getattr(n, "allocate:smartpointeraccess"));
...
int Language::memberfunctionHandler(Node *n) {
  ...
  if (directorsEnabled() && is_member_director(CurrentClass, n) && !SmartPointerEmit)
  ...
  Swig_MethodToFunction(n, NSpace, ClassType, Getattr(n, "template") ? SmartPointerEmit : Extend | SmartPointerEmit | DirectorExtraCall, director_type,
...
int Language::variableHandler(Node *n) {
  ...
  if (!CurrentClass) {
    ...
    if (Swig_storage_isstatic(n) && !(SmartPointerEmit && Getattr(n, "allocate:smartpointeraccess"))) {
...
int Language::membervariableHandler(Node *n) {
  ...
  if (!AttributeFunctionGet) {
    ...
    if (assignable) {
      ...
      int flags = Extend | SmartPointerEmit | use_naturalvar_mode(n);
      if (isNonVirtualProtectedAccess(n))
...
int Language::staticmembervariableHandler(Node *n) {
  ...
  String *value = Getattr(n, "value");
  String *classname = !SmartPointerEmit ? (isNonVirtualProtectedAccess(n) ? DirectorClassName : ClassName) : Getattr(CurrentClass, "allocate:smartpointerpointeeclassname");
...
int Language::classHandler(Node *n) {
  ...
  if (Getattr(n, "allocate:smartpointer")) {
    ...
    cplus_mode = PUBLIC;
    SmartPointerEmit = CWRAP_SMART_POINTER;
    if (Getattr(n, "allocate:smartpointerconst") && Getattr(n, "allocate:smartpointermutable")) {
      SmartPointerEmit |= CWRAP_SMART_POINTER_OVERLOAD;
    }
    Iterator c;
  ...
  if (!Abstract) {
    ...
    if ((over) && (!overloading)) {
      ...
      if (Getattr(n, "copy_constructor") && (!Getattr(CurrentClass, "has_copy_constructor"))) {
        ...
        if (false && (over != n)) { // [Bruno Levy] allow constructor overloading
