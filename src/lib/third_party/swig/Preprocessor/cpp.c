...
#include <swig/Swig/swig.h>
#include <swig/Preprocessor/preprocessor.h>
#include <ctype.h>
...
/* [Bruno Levy] Added mechanism for processing comments, like Doxygen */

static PreprocessorCommentCB preprocessor_comment_CB = NULL;

void Preprocessor_register_comment_CB(PreprocessorCommentCB cb) {
    preprocessor_comment_CB = cb;
}

static String* parsed_comment = NULL;

static void parsed_comment_begin() {
    if(preprocessor_comment_CB == NULL) {
        return;
    }
    assert(parsed_comment == NULL);
    parsed_comment = NewStringEmpty();
}

static void parsed_comment_putc(char c) {
    if(preprocessor_comment_CB == NULL) {
        return;
    }
    assert(parsed_comment != NULL);
    Putc(c,parsed_comment);
}

static void parsed_comment_end(String* preprocessor_output) {
    if(preprocessor_comment_CB == NULL) {
        return;
    }
    preprocessor_comment_CB(parsed_comment, preprocessor_output);
    Delete(parsed_comment);
    parsed_comment = NULL;
}

/* [Bruno Levy] End of added code */
